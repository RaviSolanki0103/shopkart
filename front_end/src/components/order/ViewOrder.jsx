...
const ViewOrder = () => {
  ...
  const [paginationData, setPaginationData] = useState({currentPage: 0, totalData: 0, pageSize: 2});
  ...
  // filter options
  const options = [{label: "Ordered", value: "Ordered"},
  ...
  const fetchOrderProducts = (_page,_name, _status) => {
    ...
    .then(res=>{
        ...
        if(result.status === 401) {
          ...
          Toast({msg: "Unauthorized access !!!, please login again !!!", success: false});
          navigate("/");
  ...
  const getListOfProduct = (result) => {
    ...
    result.docs.map(order=>
      ...
      order.products.map(product=>
        ...
        orderProductData.push({...product, status: order.status})
      )
    );
  ...
  return (
    <Layout className="order-page-layout">
      <Sider theme="light" className="order-filter">
        <Card size="small" title="Filter" extra={<Link to="#" onClick={clearAll}>clear all</Link>}>
          <Collapse defaultActiveKey={["1"]} onChange={callback}>
            <Panel header="Order Status" key="1">
              <Checkbox.Group className="order-filter-checkbox-group" options={options} value={checkedList} onChange={onChange} />
            </Panel>
          </Collapse>
        </Card>
      </Sider>
      <Content className="order-page-content">
            ...
            {productData.length !== 0 ? productData.map((singleProduct, index) => (
              <Card.Grid key={index} className="order-grid">
                <Row gutter={[16, 16]} align="middle">
                  ...
                  <Col span={6}>
                    <h2 className="order-product-name">{singleProduct.name}</h2>
        ...
        <div className="order-pagination">
          <Pagination current={paginationData.currentPage} defaultPageSize={paginationData.pageSize} onChange={onPageChange} total={paginationData.totalData} />
        </div>
      </Content>
    </Layout>
  );
